...
export default function SettingsPage() {
	...
	const toggleIndex = (id: string) => {
		setSettings((prev) => ({
			...prev,
			selectedIndices: prev.selectedIndices.includes(id)
				? prev.selectedIndices.filter((idx) => idx !== id)
				: [...prev.selectedIndices, id],
		}));
	};
	...
	return (
		<div className="max-w-6xl mx-auto p-4 md:p-6 lg:p-8 animate-in fade-in duration-500">
			...
			<div className="flex flex-col md:flex-row gap-8">
				{/* LEWA KOLUMNA: Nawigacja po ustawieniach */}
				...
				{/* PRAWA KOLUMNA: Zawartość zakładki */}
				<main className="flex-1 space-y-8">
					...
					{activeTab === "dashboard" && (
						<div className="space-y-6 animate-in slide-in-from-right-4 duration-500 fade-in">
							...
							<SettingsSection
								title="Obserwowane Rynki"
								desc="Wybierz indeksy i rynki, które chcesz śledzić na głównym pulpicie."
							>
								<div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
									{AVAILABLE_INDICES.map((idx) => {
										const isSelected = settings.selectedIndices.includes(
											idx.id,
										);
										return (
											<div
												...
												className={cn(
													...
													isSelected
														? "bg-blue-500/10 border-blue-500/30 shadow-[0_0_15px_-3px_rgba(59,130,246,0.15)]"
														: "bg-t-bg-panel border-t-border-subtle hover:border-slate-500/30",
												)}
											>
												<div>
													<p
														className={cn(
															...
															isSelected
																? "text-blue-500"
																: "text-t-text-secondary group-hover:text-t-text-primary",
														)}
													>
														{idx.name}
													</p>
													<p className="text-[10px] font-mono text-t-text-tertiary">
														{idx.symbol}
													</p>
												</div>
												<div
													className={cn(
														...
														isSelected
															...
															: "border-t-border-subtle bg-black/5 dark:bg-white/5",
													)}
												>
					...
					{/* ZAKŁADKA 2: PREFERENCJE (Nieaktywne z odznaką) */}
					...
					{/* POZOSTAŁE ZAKŁADKI (Placeholdery) */}
...
// =========================================================
// KOMPONENTY POMOCNICZE (Premium UI)
// =========================================================
...
function ToggleRow({ title, desc, isActive, onClick }: any) {
	return (
		...
		>
			...
			{/* Poprawiony Premium Toggle Switch */}
			<div
				className={cn(
			...
			>
				<div
					className={cn(
						...
						isActive ? "translate-x-5" : "translate-x-0", // Idealne przesunięcie o 20px
					)}
				/>
			</div>
		</div>
